...
export function http(req, serviceName, operationName) {
  return (request) => {
    ...
    const { buildRequestLogs, logSuccess, logFailure } =
      getContainer("logging");
...
export function createOpenAPIClient(name, spec) {
  ...
  const { baseUrl, timeout, retries, namingOverride, namingPath, namingQuery } =
    config;
  ...
  if (!baseUrl && !metadata.testing && !metadata.debug) {
    throw new Error(
      `OpenAPI client ${name} does not have a configured baseUrl`
    );
  }
