  ...
  keys.forEach((key) => {
    ...
    if (typeof value === "string") {
      ...
      const expandedValue = value
        ...
        .join("/");
      const newPath = expandPathWithKeyValue(
        expandedPath,
        key,
        expandedValue,
        options
...
export default (context, req, args, options) => {
  const builders = mapValues(DEFAULT_BUILDERS, (value, key) =>
    get(context, `options.build${capitalize(key)}`, value)
  );

  const baseRequest = get(
    context,
    "options.buildBaseRequest",
    buildBaseRequest
  )(context, req, args, options);
