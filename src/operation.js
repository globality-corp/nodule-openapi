...
export default (context, name, operationName) => async (req, args, options) => {
  ...
  const http = get(context, "options.http", () => axios)(
    req,
    name,
    operationName
  );
  ...
  const request = buildRequest(requestContext, req, args, options);
  ...
  const retriesCount = isMutationOperation(request)
    ? 0
    : get(request, "retries", 0);

  while (
    proxyErrorsCount <= proxyRetriesCount &&
    serviceErrorsCount <= retriesCount
  ) {
    try {
