/**
 * This is a copied ApiClient from a v2 openapi code generated api client
 *
 */
...
class ApiClient {
    /**
      ...
      */
    constructor(basePath = '/api/v2', timeout = 60000) {
        /**
          * The base URL against which to resolve every API call's (relative) path.
          * @type {String}
          * @default /api/v2
    ...
    applyAuthToRequest(request, authNames) {
        authNames.forEach((authName) => {
            ...
            switch (auth.type) {
                ...
                case 'bearer':
                    if (auth.accessToken) {
                        const localVarBearerToken = typeof auth.accessToken === 'function'
                            ? auth.accessToken()
                            : auth.accessToken;
