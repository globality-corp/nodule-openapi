...
function buildRequiredParameters(operation, filterFunc, options) {
  return get(operation, "parameters", [])
    .filter(filterFunc)
    .reduce(
      (obj, parameter) =>
        Object.assign(obj, {
          [nameFor(parameter.name, parameter.in, false, options)]:
            parameter.required,
        }),
      {}
    );
...
function validateQueryAndPathParameters(
  operation,
  operationName,
  args,
  options
) {
  const parameters = buildRequiredParameters(
    operation,
    isQueryOrPathParameter,
    options
  );
...
export default ({ spec, method, path, options }) =>
  (req, operationName, args) => {
