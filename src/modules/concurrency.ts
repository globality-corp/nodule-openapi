...
function getConcurrency(concurrencyLimit: number | null) {
    ...
    return parseInt((getConfig<Config, 'concurrency.limit'>('concurrency.limit') ?? DEFAULT_CONCURRENCY).toString(), 10);
}


export default function concurrentPaginate<PromiseResult>(promises:  Promise<PromiseResult>[], concurrencyLimit: number | null = null): Promise<PromiseResult[]> {
    ...
    return Promise.all(
        // @ts-expect-error throat types do not support this call
        promises.map(promise => funneledThroat(() => promise))  as unknown as Promise<PromiseResult>[],
    );
