...
export async function allForBodySearchRequest<Context, Args, Result>(
  ...
  const defaultLimit =
    getConfig<Config, "defaultLmit">("defaultLmit") ?? DEFAULT_LIMIT;
  ...
  let params = {
    body: {
      ...
      offset: offset || 0,
    } as Args & Partial<BaseRequestArgs>,
  };
  const firstPage = await searchRequest(req, params);

  if (
    isNil(firstPage.offset) ||
    isNil(firstPage.limit) ||
    isNil(firstPage.count)
  ) {
...
export default async function all<Context, Args, Result>(
  ...
  if (limit === defaultLimit && firstPage.count > DEFAULT_PAGING_UPPER_BOUND) {
    ...
    logger.warning(req, "A large dataset requested with default limit set.", {
      searchRequest,
  ...
  const nextPages = await concurrentPaginate(
    offsets.map((pageOffset) =>
      searchRequest(req, { ...params, offset: pageOffset })
    ),
    concurrencyLimit
  );
  return flatten([firstPage, ...nextPages].map((page) => page.items));
