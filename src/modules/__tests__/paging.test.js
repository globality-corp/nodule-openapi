...
describe("Pagination", () => {
  ...
  it("test none", async () => {
    const res = await none(req, {
      searchRequest: searchRequestNone,
      args: { param: "eter" },
    });
  ...
  it("test none: too many results", async () => {
    ...
    try {
      expect(
        await none(req, {
          searchRequest: searchRequestTwo,
          args: { param: "eter" },
        })
      ).toThrow();
    } catch (thrownError) {
